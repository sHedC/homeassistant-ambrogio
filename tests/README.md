...
There are two options for doing unit testing with py test:

The first is to alter the .devcontainer.json file to replace the BUILD_TYPE with dev and rebuild the container:
...
Alternatively you can create a virtual environment to install dependencies:
```bash
python3 -m venv venv
...
These options will install `homeassistant`, `pytest`, and `pytest-homeassistant-custom-component`, a plugin which allows you to leverage helpers that are available in Home Assistant for core integration tests.
